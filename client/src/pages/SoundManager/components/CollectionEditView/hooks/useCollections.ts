...
export const useCollections = ({
  ...
  const handleAddItems = useCallback(async (audioItems: AudioItem | AudioItem[], position?: number) => {
    ...
    const newItems = itemsArray.filter(item => 
      !collectionItems.some(existing => existing.id === item.id)
    );
    ...
    setCollectionItems([...updatedItems, ...itemsToInsert].sort((a, b) => 
  ...
  const handleRemoveItems = useCallback(async (itemIds: number | number[]) => {
    ...
    const itemsToRemove = collectionItems.filter(item => idsArray.includes(item.id));
    ...
    const remainingItems = collectionItems.filter(item => !idsArray.includes(item.id));
    ...
    try {
      ...
      setCollectionItems(collectionItems);
      setError(`Failed to remove items from ${collectionType}. Please try again.`);
    }
  ...
  const handleUpdateItemPositions = useCallback(async (
    itemId: number,
    targetPosition: number,
    sourceStartPosition?: number,
    sourceEndPosition?: number
    ...
    if (sourceStartPosition !== undefined && sourceEndPosition !== undefined) {
      ...
      itemsToMove = updatedItems.filter(item => {
        ...
        return pos !== undefined && pos >= sourceStartPosition && pos <= sourceEndPosition;
      });
    ...
    if (targetPosition > endPos) {
      ...
      for (const item of updatedItems) {
        ...
        if (
          pos !== undefined &&
           targetPosition <= pos && pos < startPos &&
          !itemsToMove.includes(item)
        ) {
          item.position = pos + itemCount;
        }
    ...
    try {
      ...
      setCollectionItems(collectionItems);
      setError(`Failed to update positions in ${collectionType}. Please try again.`);
    }
