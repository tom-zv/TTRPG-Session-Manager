 ...
 */
export async function downloadYouTubePlaylist(fileData: FileData, folderPath: string) {
  ...
  const downloadPromises = entries.map((entry, i) => {
    return limit(async () => {
      ...
      if (!videoUrl) return; // Skip if no URL
      ...
      try {
        ...
        if (newFileData) {
          parentPort?.postMessage({ type:'progress', file: newFileData, index: i + 1, total });
        }
      } catch (err) {
