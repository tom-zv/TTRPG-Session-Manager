import { getAllAudioFiles, insertAudioFiles } from "src/api/audio/files/fileService.js";
...
import folderModel, { FolderInsertData } from "src/api/audio/folders/folderModel.js";
import { createFolder } from "src/api/audio/folders/folderModel.js";
...
const AUDIO_EXTENSIONS = [
  '.mp3', '.mpeg', '.opus', '.m4a', '.ogg', '.oga', '.wav', 
  '.aac', '.caf', '.mp4', '.weba', '.webm', 
  '.dolby', '.flac'
];
...
export async function syncAudioLibrary(): Promise<{
  ...
  try {
    ...
    // Get the next available insert ID for folders
    const [rows] = await audioPool.execute(
      `SELECT COALESCE(MAX(id),0) + 1 AS nextId FROM folders;`
    );
    const currentInsertId = (rows as RowDataPacket[])[0].nextId;
    ...
    const { folderInsertArray, fileInsertArray: fileArray } = result;
    ...
    try {
      if (folderInsertArray.length > 0) {
        await Promise.all(folderInsertArray.map(folder => createFolder(folder, connection)));
      }
      ...
      return {
        ...
        foldersAdded: folderInsertArray.length,
      ...
      if (error instanceof Error) {
        throw new Error(`Failed to synchronize audio library: ${error.message}`);
      } else {
        throw new Error(`Failed to synchronize audio library: ${String(error)}`);
      }
    } finally {
      connection.release();
    }
...
async function scanFsAndBuildInsertArray(
  ...
  folderInsertArray: FolderInsertData[];
  fileInsertArray: FileInsertData[];
}> {
  const folderInsertArray: FolderInsertData[] = [];
  ...
  while (queue.length) {
    ...
    if (existingFolder) {
      ...
      // if this is a new folder, assign the next insert id to it, and increment it.
      //console.log(`Inserting new folder: ${dirPath}, rel: ${rel_path}, type: ${audioType}`);

      if( parentId == null){
        // This means the root folder isnt in the DB, error in the folder structure.
        throw new Error(`Root folder ${rootPath} is not in the database!`);
      }

      folderId = nextInsertId++
      folderInsertArray.push({
  ...
  return { folderInsertArray, fileInsertArray };
}
 ...
 */
async function fetchFileDurations(files: FileInsertData[]): Promise<FileInsertData[]> {
